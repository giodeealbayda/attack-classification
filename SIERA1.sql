...
CREATE INDEX `attackerID_idx` ON `mydb`.`attack` (`attackerID` ASC);

CREATE INDEX `victimID_idx` ON `mydb`.`attack` (`victimID` ASC);
...
CREATE INDEX `fk_response_1_idx` ON `mydb`.`response` (`attackID` ASC);
...
CREATE TABLE IF NOT EXISTS `mydb`.`TCP_Reset` (
  ...
  CONSTRAINT `TCP_Reset-responseID`
...
CREATE INDEX `responseID_idx` ON `mydb`.`TCP_Reset` (`responseID` ASC);
...
CREATE TABLE IF NOT EXISTS `mydb`.`Timebased_Block` (
  ...
  CONSTRAINT `Timebased_Block-responseID`
    FOREIGN KEY (`responseID`)
    REFERENCES `mydb`.`response` (`responseID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE INDEX `responseID_idx` ON `mydb`.`Timebased_Block` (`responseID` ASC);
...
CREATE TABLE IF NOT EXISTS `mydb`.`Permanent_Block` (
  ...
  CONSTRAINT `Permanent_Block-responseID`
    FOREIGN KEY (`responseID`)
    REFERENCES `mydb`.`response` (`responseID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE INDEX `responseID_idx` ON `mydb`.`Permanent_Block` (`responseID` ASC);
...
CREATE INDEX `victimID_idx` ON `mydb`.`high_priority_users` (`victimID` ASC);
...
CREATE TABLE IF NOT EXISTS `mydb`.`attack_persistence` (
  ...
  CONSTRAINT `attack_persistence-attackID`
    FOREIGN KEY (`attackID`)
    REFERENCES `mydb`.`attack` (`attackID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE INDEX `attackID_idx` ON `mydb`.`attack_persistence` (`attackID` ASC);
...
CREATE TABLE IF NOT EXISTS `mydb`.`time_interval_persistence` (
  ...
  CONSTRAINT `time_interval_persistence-attack_persistenceID`
...
CREATE INDEX `attack_persistenceID_idx` ON `mydb`.`time_interval_persistence` (`attack_persistenceID` ASC);
